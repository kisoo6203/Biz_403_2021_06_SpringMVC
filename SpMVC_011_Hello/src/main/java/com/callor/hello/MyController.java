...
public class MyController {
	 ...
	 *  	DELETE : 삭제(DELETE)
	 *  
	 *  RequestMapping과 RequestMethod가 동시에 같은경우는 절대 허용하지 않는다
	 *  RequestMapping이 같은데 RequestMethod가 다르면 허용한다
	 *  
	 *  GET method 방식으로 form에 담긴 데이터를 전송할때
	 *  http://localhost:8080/hello/insert?name=홍길동&tel=전화번호
	 *  - 전송하는 데이터가 주소에 포함되어서 전송되기때문에
	 *    데이터가 쉽게 노출될 수 있다
	 ...
	 *  method가=POST만 있을 경우 서버는 405 Http 상태코드(Status Code)를
	 *  응답한다.
	 */
	 ...
	 * 		숫자형, Boolean형 등 - (String형이 아닌것)
	 * 설정을 하면 Reflection이 실행되는 동안에 String형 데이터를
	 * 	설정된 데이터 type으로 형변환 과정이 실행된다
	 * 이 과정에서 형변환의 Exception이 발생하면
	 * Reflection 코드가 중단되고 Apring의 Dispatcher는 Web 400 status Code를
	 * 응답한다.
	 * 
	 * 또한 구체적은 어떤 문제가 발생했는지 알려주지않는다
	 * 
	 */
