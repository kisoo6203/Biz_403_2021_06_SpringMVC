 ...
 * 1. 상속되는 부모클래스의 역할
 * 		- 스스로는 객체로 생성될 수 없다.
 * 2. 추상메서드를 포함하는데 상속받은 곳에서는 반드시 추상메서드를 구현해야 한다.
 * 3. 상속받은 클래스들에 필요한 공통처리 method는 일반 클래스처럼 자체적으로 구현코드를 가지고 있다.
 * 4. 공통처리 method도 필요에 따라 상속받는 곳에서 재 정의 할 수 있다.
 * 5. 인터페이스의 역할을 수행
 * 		- 여러 클래스의 원형(prototype)역할을 수행하면서 상속받은 클래스들에 공통으로 필요한 코드를 미리 제공한다
 * 
 * 객체 생성하는 방법
 * 추상클래스 객체 = new 상속받은 클래스()
 * 인터페이스 객체 = new imp클래스()
 * 
 * 추상클래스 객체 = new 추살클래스() 사용불가
 * 
 */
