...
public class CompDaoImplV1 implements CompDao {
	...
	/*
	 * jdbcTemplate을 사용하여 QUERY를 실행할 때ㅣ 각 method에서 매개변수를 받아 QUERY에 전달할텐데 이때 매개변수는
	 * primitive로 받으면 값을 변환시키는 어려움이 있다 그래서 권장사항으로 매개변수는 wrapper class type으로 설정하는게
	 * 좋다 즉 숫자형일 경우 int, long 대신 INTEGER, Long 형으로 선언
	 * 
	 * vo에 담겨서 전달된 값은 Object[] 배열로 변환한 후 jdbcTemplate에 전달해 주어야 한다
	 * 
	 * 하지만, 1 ~ 2개 정도의 값을 전달할 때 Object[] 배열로 변환 후 전달을 하면 Object 객체 저장소가 생성되고 메모리를
	 * 사용한다 이때 전달되는 변수가 wrapper class type 이면 Object[]로 만들지않고 바로 주입할 수 있다
	 */
	@Override
