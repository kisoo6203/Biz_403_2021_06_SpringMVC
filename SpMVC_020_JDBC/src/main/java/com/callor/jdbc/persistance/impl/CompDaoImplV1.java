...
public class CompDaoImplV1 implements CompDao {
	...
	public List<CompVO> selectAll() {
		...
		jdbcTemplate.query(sql, new BeanPropertyRowMapper<CompVO>(CompVO.class));
	...
	/*
	 * jdbcTemplate을 사용하여 QUERY를 실행할 때ㅣ
	 * 각 method에서 매개변수를 받아
	 * QUERY에 전달할텐데
	 * 이때 매개변수는 primitive로 받으면 값을 변환시키는
	 * 어려움이 있다
	 * 그래서 권장사항으로 매개변수는 wrapper class type으로 설정하는게 좋다
